...
- [6. 核心运行链路](#核心运行链路)
  - [6.1 建图流程](#1-建图流程)
  - [6.2 导航-巡航赛流程](#2-导航--巡航赛流程)
- [7. 整体架构解析](#整体架构解析)
  - [7.1 底盘层：abot_base](#a-底盘层abot_base)
  - [7.2 SLAM-定位-导航：robot_slam--jie_ware--pid_follow_planner](#b-slam--定位--导航robot_slam--jie_ware--pid_follow_planner)
  - [7.3 比赛任务编排：robot_slam-与-user_demo](#c-比赛任务编排robot_slam-与-user_demo)
  - [7.4 视觉感知模块](#d-视觉感知模块)
  - [7.5 语音与外设扩展](#e-语音与外设扩展)
- [8. 主要的话题-服务-数据流](#主要的话题--服务--数据流)
- [9. 主要包说明](#主要包说明)
- [10. 快速开始](#快速开始)
- [11. 可能的移植注意事项](#可能的移植注意事项)
...
tips：为应对当年国赛新规则的改变（区块识别结果会产生的链式影响），我们在launch里加上了可以进行绝境争分的**狂暴模式**，这个版本启动后会抓住官方规则中的漏洞（毕竟官方规则可没说识别必须按顺序来执行嘻嘻），实时证明这个模式确实能比别人的跑法要快约20s左右。

这也是我仔细设计的比赛逻辑，不是简单的单点导航，而是**场地分区块末端制导解耦合多任务巡航序列**。
...
## ⚠️ 维护说明 / Maintenance Notice

本项目作为中国机器人及人工智能大赛（CRAIC）的冠军方案快照，目前计划将暂时处于**停止活跃维护**状态。

* **🛑 停止更新：** 受限于个人精力与毕业/升学规划，本项目将不再跟进后续的 Bug 修复、新版本 ROS 适配、新硬件测试或未来赛事规则的变更。
* **💡 交流探讨：** 非常欢迎大家通过 [Issues] 探讨技术方案、调车思路与学习方法，我会尽量抽空回复与交流。
* **🔕 关于 PR：** 鼓励大家基于本项目进行 Fork 和二次迭代，但提交至本仓库的 Pull Request (PR) 可能不会被审阅或合并。

希望这我份从零打磨的代码，能为你的比赛和机器人学习之路提供有价值的参考。祝武运昌隆！
